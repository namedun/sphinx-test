...
# Variables substitutions. Uses {VAR} subst using variables defined in tanowrt.yaml
...
def yocto_vars_env_get_outdated(app: Sphinx, env, added, changed, removed):
    '''
    If tanowrt.yaml changed (BUILDDIR/.tanowrt.yaml.cache does not exist or contains
    an md5sum different from tanowrt.yaml's current md5sum), force rebuild of all
    *.rst files in SOURCEDIR whose content has at least one occurence of `&.*;`
    (see PATTERN global variable).
    '''
    try:
        poky_cache = Path(app.outdir) / ".tanowrt.yaml.cache"
